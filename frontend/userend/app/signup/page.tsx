...
function Signup() {
 ...
 body:new URLSearchParams({email,mode:"signup"})
  ...
  const handleSubmit=async ()=>{
    ...
    body:new URLSearchParams({name,email,finalOtp})
    });
    const data=await result.json();
    if(data.success){
...
useEffect(() => {
  ...
  gutter={8} toastOptions={{duration:5000,    style: {
      ...
      <div className='w-[80vw] h-[55vh]   sm:w-[70vw] md:w-[60vw] lg:w-[50vw]   xl:w-[40vw]  lg:h-[60vh] flex flex-col  border-2 rounded-[40px] sm:rounded-[50px] shadow-[0.8px_8px_25px] shadow-blue-500  bg-gray-700 p-4'>
   ...
   { step===1 &&   <div>
    <div className='flex flex-col gap-5 mt-10 lg:mt-15 xl:mt-20 '>
         <div className='relative  flex flex-row  justify-center items-center'>
      <input type="text" placeholder='Enter your name' value={name} onChange={handleChangeName} required className='pl-10 sm:pl-12 lg:pl-13 xl:pl-15 text-[0.8rem] sm:text-[1rem] lg:text-[1.1rem] xl:text-[1.2rem] font-serif outline-2 rounded-xl w-[70vw] sm:w-[60vw] md:w-[50vw] lg:w-[40vw] xl:w-[30vw] h-[5vh] bg-white'/>
      ...
      <input type="email" placeholder='jhondoe@gmail.com' value={email} onChange={handleChangeMail} required className='pl-10 sm:pl-12 lg:pl-13 xl:pl-15 text-[0.8rem] sm:text-[1rem] lg:text-[1.1rem] xl:text-[1.2rem] font-serif outline-2 rounded-xl w-[70vw] sm:w-[60vw] md:w-[50vw] lg:w-[40vw] xl:w-[30vw] h-[5vh] bg-white'/>
       ...
       <div className='flex flex-row justify-center items-center mt-10 xl:mt-20'>
    ...
    if (e.key === "Backspace") {
      ...
      if (index > 0) inputRef.current[index - 1]?.focus();
              ...
              <div className='flex flex-row justify-center items-center mt-5 lg:mt-10'>
                <motion.button whileTap={{scale:0.9}} transition={{duration:0.15,ease:"easeIn"}} className={`w-[40vw] sm:w-[30vw] md:w-[25vw]  lg:w-[20vw] h-[5vh] rounded-xl text-[1rem] sm:text-[1.2rem] lg:text-[1.3rem] xl:text-[1.5rem] font-mono text-gray-800 cursor-pointer flex justify-center items-center ${loading ? "bg-blue-200" : "bg-blue-400"}`} onClick={()=>{handleSubmit()}}>{!loading ? "Submit" : <Spinner/>}</motion.button>
              </div>
            </div>
            </div>
        }
     {step===1 &&    <div className='flex flex-row justify-center items-center gap-1 xl:gap-2 text-[0.8rem] sm:text-[1rem] md:text-[1.2rem] lg:text-[1.3rem] xl:text-[1.5rem] mt-5 lg:mt-7'>
